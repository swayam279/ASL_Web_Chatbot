...
from chatbot import Chat_Stream, build_chat_history
from run import (
...
def extract_sources(answer: str) -> list[str]:
    """
    Parses source URLs from the model's answer text.
    The QA prompt instructs the model to always end with:
        **Sources:**
        - <url>
    This function extracts those URLs for display in the expander.
    Used instead of get_source_urls(docs) since streaming does not
    return docs — only the text chunks as they arrive.
    """
    sources = []
    in_sources = False
...
with st.sidebar:
    ...
    with pcol:
        if active_base_url:
            if st.button("＋", help="New chat", use_container_width=True):
                st.session_state.session_id = create_conversation(active_base_url)
                st.session_state.show_rename = False
                st.rerun()
    ...
    if conversations:
        ...
        if st.session_state.session_id and st.session_state.show_rename:
            st.divider()
            ...
            with rcol1:
                if st.button("Save", use_container_width=True) and new_name.strip():
                    rename_conversation(st.session_state.session_id, new_name.strip())
                    st.session_state.show_rename = False
                    st.rerun()
            with rcol2:
...
if not st.session_state.session_id:
    ...
    st.markdown(
        ...
        "The app indexes every page once, then answers your questions "
    ...
    if user_input:
        ...
        # st.write_stream() consumes the Chat_stream generator and renders
        # tokens progressively as they arrive. It returns the full assembled
        # string when the stream is complete — used for saving and source parsing.
        with st.chat_message("assistant"):
            streamed_answer = st.write_stream(
                Chat_Stream(retriever, base_url, user_input, chat_history)
            )

            # Extract and display sources from the completed answer text.
            # The model is prompted to always end with **Sources:** - <url>
            # so we parse from the answer rather than needing a separate docs list.
            sources = extract_sources(streamed_answer)
            if sources:
                with st.expander("Sources", expanded=False):
                    for src in sources:
                        st.markdown(f"- [{src}]({src})")

        # Save both messages to the in-memory store after rendering
        save_message(session_id, "user", user_input)
        save_message(session_id, "assistant", streamed_answer)